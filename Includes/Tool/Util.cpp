...
namespace Util
{
    ...
    std::string extractClassNameFromTypename(const char *typeName)
    {
        ...
        if (ltIndex != std::string::npos && dotIndex > ltIndex)
    ...
    {
        static auto path = UnityResolve::getDataPath();
...
namespace ImGui
{
    ...
    bool IsItemHeld(float holdTime)
    {
        ...
        if (ImGui::IsItemActive() && g.HoveredIdTimer >= holdTime)
            return true;
        return false;
    }
    ...
    public:
        ...
        const std::vector<float> &getFpsBuffer() const { return fpsBuffer; }
    private:
        int bufferSize;
        int currentFrame;
